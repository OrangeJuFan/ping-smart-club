...
1. 在 Controller 类上使用 `@Tag` 注解标注接口分组
2. 在方法上使用 `@Operation` 注解标注接口说明
3. 在参数上使用 `@Parameter` 注解标注参数说明
4. 在实体类上使用 `@Schema` 注解标注模型说明

### 示例代码

```java
@Tag(name = "教练管理", description = "ping-smart-club-admin-vue2-webpack:上传后台管理系统（vue2+webpack）教练相关的接口")
@RestController
@RequestMapping("/coach")
public class CoachController {
    
    @Operation(summary = "获取教练列表", description = "分页查询所有教练")
    @GetMapping("/list")
    public List<Coach> list() {
        // ...
    }
